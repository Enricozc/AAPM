...
class Armario(Base):
    ...
    localizacao   = Column(String(100), nullable=True)   # ex: "Bloco A - 2º andar"
    ocupado       = Column(Boolean, default=False)
    ativo         = Column(Boolean, default=True)

    # Usuário que está usando atualmente
    ...
    # Histórico (1 armário → muitos registros)
...
class ArmarioHistorico(Base):
    ...
    usuario_id   = Column(Integer, ForeignKey("usuarios.id", ondelete="SET NULL"), nullable=True)
    usuario_nome = Column(String(100), nullable=True)   # salvo por segurança, mesmo se o user for deletado
    acao         = Column(String(20), nullable=False)   # "ATRIBUIDO" ou "LIBERADO"
